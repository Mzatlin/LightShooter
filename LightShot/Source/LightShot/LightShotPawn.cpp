...
const FName ALightShotPawn::MoveForwardBinding("MoveForward");
const FName ALightShotPawn::MoveRightBinding("MoveRight");
const FName ALightShotPawn::FireForwardBinding("FireForward");
const FName ALightShotPawn::FireRightBinding("FireRight");

ALightShotPawn::ALightShotPawn()
{	
...
void ALightShotPawn::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	if (bCanFire == true)
	{
		...
		if (FireDirection.SizeSquared() > 0.0f)
		{
			...
			if (World != nullptr)
			{
				...
				World->SpawnActor<ALightShotProjectile>(SpawnLocation, FireRotation);
			}

			bCanFire = false;
			World->GetTimerManager().SetTimer(TimerHandle_ShotTimerExpired, this, &ALightShotPawn::ShotTimerExpired, FireRate);
...
void ALightShotPawn::ShotTimerExpired()
{
	bCanFire = true;
}
